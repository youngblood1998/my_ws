...
def delete_model(row,col,model_name):
    ...
    try:
        ...
        for i in range(0,row*col):
            remove_model_proxy(model_name+"_"+str(i+3)+"cm")
...
def add_model(row,col,model_name,x,width,y,height):
    ...
    for i in range(0,row):
        for j in range(0,col):
            ...
            name=model_name + "_" + str(i*col+j+3) + "cm"
            os.system("rosrun gazebo_ros spawn_model -file " + GAZEBO_MODEL_PATH + name +"/model.sdf -sdf -model " + name +" -x "+str(pos_x)+" -y "+str(pos_y)+" -z 0.63 -Y "+str(3.14*random()))

def main():
    rospy.init_node('model_change', anonymous=True)
    delete_model(2,2,"wood_cube")
    add_model(2,2,"wood_cube",0.36,0.46,-0.2,0.62)
